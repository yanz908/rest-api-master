...
router.get('/downloader/ytplay', async(req, res, next) => {
  ...
  if(listkeyprem.includes(apikey)){
    fetch(encodeURI(`https://yanz-api-master.herokuapp.com//api/downloader/ytplay?query=${query}&apikey=${global.apikey}`))
...
router.get('/cecan/random', async (req, res, next) => {
        ...
        fetch(encodeURI(`https://raw.githubusercontent.com/yanz908/dtbs/main/cecan/random.json`))
        .then(response => response.json())
        .then(async data => {
        var result = data[Math.floor(Math.random() * data.length)];
        var buffer = result.url;
...
router.get('/search/youtube', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://yanz-api-master.herokuapp.com//api/search/youtube?query=${query}&apikey=danzz`))
    .then(response => response.json())
        .then(hasil => {
